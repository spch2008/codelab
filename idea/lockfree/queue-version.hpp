  ...
  bool push(node* new_node) {
    for (;;) {
      ...
      if (old_tail_next == nullptr) {
        ...
        TagPtr<node> new_tail(old_tail_next, old_tail.GetNextTag());
        tail_.compare_exchange_strong(old_tail, new_tail);
      }
    }
  }
  ...
  T* pop () {
    ...
    for (;;) {
      ...
      if (old_tail.GetPtr() == old_head.GetPtr()) {
        ...
        TagPtr<node> new_tail(old_head_next, old_tail.GetNextTag());
        tail_.compare_exchange_string(old_tail, new_tail);
      } else {
        TagPtr<node> new_head(old_head_next, old_head.GetNextTag());
