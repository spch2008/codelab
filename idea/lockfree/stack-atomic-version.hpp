...
class Stack {
 public:
  ...
  Stack() : head_(TagPtr<Node>(nullptr, 0)) {}

  void Push(Node* node) {
    TagPtr<Node> old_head = head_.load(memory_order_acquire);
    for (;;) {
      TagPtr<Node> new_head(node, old_head.GetTag());
      new_head->next = old_head.GetPtr();
  ...
  Node* Pop() {
    ...
    for (;;) {
      ...
      TagPtr<Node> new_head(old_head->next, old_head.GetNextTag());
      if (head_.compare_exchange_weak(old_head, new_head)) {
        break;
      }
    }

    return old_head.GetPtr();
  }
